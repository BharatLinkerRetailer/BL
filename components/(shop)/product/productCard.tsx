...
import { useCart } from "../../../store/cart/cartStore";   // ← Added
...
export function ProductCard({
  ...
  const isOutOfStock = variant.status === "out_of_stock" || variant.status === "discontinued";

  // Variant attributes (e.g. "500ml • Red")
  ...
  const handleQuantityChange = (newQty: number) => {
    ...
    if (cartItem) {
      ...
      // Your useCart store already removes the item when quantity becomes 0
    }
  };
  ...
  return (
    <TouchableOpacity style={styles.card} activeOpacity={0.93} onPress={onPress}>
      ...
      <View style={styles.imgWrapper}>
        {image ? (
          <Image source={{ uri: image.url }} style={styles.img} resizeMode="contain" />
        ) : (
...
const styles = StyleSheet.create({
  card: {
    ...
    borderColor: C.gray2,
    ...Platform.select({
      ios: { shadowColor: "#00000030", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.07, shadowRadius: 6 },
  ...
  deliveryText: { fontSize: 9, fontWeight: "700", color: C.green },
  ...
  discountText: { fontSize: 9, fontWeight: "800", color: "#fff", textAlign: "center", lineHeight: 11 },

  oosOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(255,255,255,0.72)", alignItems: "center", justifyContent: "center" },
  oosPill: { backgroundColor: "rgba(0,0,0,0.55)", borderRadius: 20, paddingHorizontal: 10, paddingVertical: 5 },
  oosText: { fontSize: 11, fontWeight: "600", color: "#fff" },

  body: { paddingHorizontal: 9, paddingTop: 7, paddingBottom: 9, gap: 4 },
  qtyPill: {
  ...
  qtyText: { fontSize: 10, fontWeight: "600", color: C.textSub },

  name: { fontSize: 12.5, fontWeight: "600", color: C.text, lineHeight: 17 },
  ...
  compare: { fontSize: 10.5, color: C.gray3, textDecorationLine: "line-through", marginTop: 1 },
  ...
  addBtnText: { fontSize: 12, fontWeight: "800", color: C.green },
  addBtnPlus: { fontSize: 14, fontWeight: "700", color: C.green, lineHeight: 16 },
  ...
  notifyBtn: { borderWidth: 1, borderColor: C.gray3, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 5 },
  notifyText: { fontSize: 11, fontWeight: "600", color: C.textSub },
  ...
  savingsText: { fontSize: 9.5, fontWeight: "700", color: C.green },
});